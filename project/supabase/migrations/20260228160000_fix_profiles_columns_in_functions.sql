-- Fix profiles inserts: table has id, role, staff_id, must_change_password only (no created_at/updated_at)

-- 1. Recreate handle_new_staff_user with correct profiles columns
...
BEGIN
  IF NEW.email IS NOT NULL AND NEW.user_id IS NULL THEN
    ...
    VALUES (
      new_user_id, NEW.email, crypt(temp_password, gen_salt('bf')), now(),
...
-- 2. Recreate create_staff_user_account with correct profiles columns
...
BEGIN
  ...
  VALUES (
    new_user_id, email_param, crypt(temp_password, gen_salt('bf')), now(),
    '{"provider":"email","providers":["email"]}',
...
-- 3. Recreate create_staff_with_login with correct profiles columns
...
BEGIN
  ...
  -- Insert staff first so staff_id exists for profiles FK
