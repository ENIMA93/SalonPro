        ...
        <div className="mb-8">
          <div className="inline-flex items-center gap-4 px-6 py-4 rounded-xl bg-gray-800 border border-gray-700">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white">{revenueToday.toFixed(2)} DH</p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
            <table className="w-full">
              ...
              <tbody>
                {sorted.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center text-gray-500 py-16">No transactions yet.</td>
                  </tr>
                ) : (
                  sorted.map((row) => (
                    <tr key={row.id} className="border-b border-gray-700 last:border-0 hover:bg-gray-700/40 transition-colors">
                      ...
                      <td className="px-6 py-4 text-white">Walk-in</td>
                      <td className="px-6 py-4 text-green-400 font-semibold">{Number(row.total_amount).toFixed(2)} DH</td>
      ...
      <div ref={printAreaRef} id="receipt-print-area" className="fixed left-[-9999px] top-0 w-[280px] bg-white text-black p-6 font-sans" style={{ display: printTransaction ? 'block' : 'none' }}>
        {printTransaction && (
          <>
            ...
            <p className="text-sm text-gray-600 mb-4">{formatTableDate(printTransaction.created_at)}</p>
            <ul className="border-b border-gray-300 pb-3 mb-3 space-y-1 text-sm">
              {(Array.isArray(printTransaction.items_json) ? printTransaction.items_json : []).map((item: { name?: string; price?: number; quantity?: number }, i: number) => (
                <li key={i} className="flex justify-between">
                  ...
                  <span>{Number((item.price ?? 0) * (item.quantity ?? 1)).toFixed(2)} DH</span>
                </li>
              ))}
            </ul>
            <div className="flex justify-between font-semibold text-sm mb-2">
              <span>Total</span>
              <span>{Number(printTransaction.total_amount).toFixed(2)} DH</span>
            </div>
      ...
      {receiptId && receiptTransaction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-gray-800 rounded-xl border border-gray-700 w-full max-w-md mx-4 p-6 shadow-xl">
            ...
            <p className="text-gray-400 text-sm mb-4">{formatTableDate(receiptTransaction.created_at)}</p>
              ...
              <ul className="space-y-2 mb-4">
                {receiptItems.map((item, i) => (
                  <li key={i} className="flex justify-between text-sm">
                    ...
                    <span className="text-gray-400">{Number((item.price ?? 0) * (item.quantity ?? 1)).toFixed(2)} DH</span>
                  </li>
                ))}
              </ul>
            ) : (
            ...
            <div className="flex justify-between font-semibold text-white pt-2 border-t border-gray-700">
              <span>Total</span>
              <span className="text-green-400">{Number(receiptTransaction.total_amount).toFixed(2)} DH</span>
            </div>
